...
function ExportPanel({ reportText, answers, onDismiss }) {
  ...
  const handlePptx = async () => {
    ...
    try {
      ...
      const darkSlide = () => { const s = pres.addSlide(); s.background = { color: NAVY }; s.addShape(pres.ShapeType.rect, { x:0, y:0, w:0.25, h:5.625, fill:{color:GOLD}, line:{color:GOLD} }); return s; };
      const lightSlide = (title, color = NAVY) => {
        const s = pres.addSlide(); s.background = { color: "F8F9FC" };
        s.addShape(pres.ShapeType.rect, { x:0, y:0, w:10, h:0.07, fill:{color}, line:{color} });
        s.addText(title, { x:0.5, y:0.18, w:9, h:0.58, fontSize:20, bold:true, color, fontFace:"Calibri" });
        return s;
      };

      const s1 = darkSlide();
      s1.addText(answers.topic || mainTitle || "Strategic Foresight Report", { x:0.45, y:0.9, w:9.3, h:1.6, fontSize:36, bold:true, color:WHITE, fontFace:"Calibri" });
      s1.addText(`A ${answers.region || ""} Strategic Foresight Report`, { x:0.45, y:2.6, w:9, h:0.5, fontSize:17, color:GOLD, fontFace:"Calibri", italic:true });
      ...
      [["OPTIMISTIC","~25%",GREEN],["BASELINE","~50%",BLUE],["DISRUPTIVE","~25%",RED]].forEach(([lbl,prob,col],i) => {
        ...
        s2.addText(blurb(findSection(lbl), 90), { x:x+0.1, y:4.4, w:2.7, h:0.6, fontSize:10, color:"DDDDDD", fontFace:"Calibri" });
      });
      ...
      s7.addText(`${answers.topic || "Strategic Foresight"}  ·  ${answers.region || ""}  ·  ${new Date().getFullYear()}`, { x:1, y:4.1, w:8, h:0.4, fontSize:13, color:MID, fontFace:"Calibri", italic:true, align:"center" });

      const blob = await pres.write({ outputType: "blob" });
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = "foresight-report.pptx";
      a.click();
      setTimeout(() => URL.revokeObjectURL(url), 10000);
    } catch (err) {
